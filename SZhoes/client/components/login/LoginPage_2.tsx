...
export function LoginPage_2({ formData, setStep }: LoginInterface) {
    ...
    return (<div className="sm:flex-row sm:flex w-3/4 mx-auto mt-20 shadow-xl shadow-neutral-300 rounded-3xl">
        ...
        <div className="flex flex-col sm:w-2/3 w-full py-10 bg-white sm:rounded-e-3xl sm:rounded-s-none rounded-2xl pl-10">
            ...
            <div className="flex flex-row w-11/12 bottom-0 mt-auto  space-x-5">
                ...
                <Button isDisabled={formData.sendOtp === formData.otp ? false : true} onClick={nextStep} variant="solid" size="md" radius="none" className="bg-green-500 mt-10 uppercase w-full self-center">Next</Button>
            </div>
        </div>
    </div>
    )
