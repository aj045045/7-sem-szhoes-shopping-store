 ...
 */
export function Chart({ title, imagePath, display = "" }: ChartInterface) {
    ...
    const { data, fetchData, status } = useGetHook(fetchQuery);

    useEffect(() => {
        fetchData();
    }, [fetchData, startEndDate, select]);
    ...
    let isSelectedCustom = select === 'CUSTOM';
    return (
        <div className="w-full p-4 my-4 space-y-3 rounded-lg bg-neutral-300 border-1 border-neutral-400">
            <div className="flex justify-between">
                <div className={`${montserrat_Subrayada.className} text-xl [word-spacing:10px] text-green-950`} id={title.toLowerCase().replace(/ /g, '-')}>{title}</div>
            ...
            <Skeleton className="p-4 mx-auto bg-white rounded-md min-w-1/2 min-h-60" isLoaded={status}>
                {status &&
