...
import { ProductCardUtil } from './product-card';
import { ProductDetailInterface, ProductInterface } from '@/interfaces/product';

/**
 * The component that is used for the product detail map wrapper of cards
 * 
 * @param products - The Product list
 * @param title - The Title of the product map
 * @returns The Slider of product card
 */
export function ProductListUtil({ products, title }: ProductDetailInterface) {
        ...
        <div className="relative w-full py-2 space-y-2 mt-10 select-none">
            ...
            <div ref={scrollContainerRef} className="w-full space-x-4 overflow-x-auto scrollbar-hide whitespace-nowrap scroll-smooth px-10">
                {products.map((product: ProductInterface, index: number) => (
                    <ProductCardUtil product={product} key={index} />
                ))}
            </div>
